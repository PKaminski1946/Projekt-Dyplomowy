...
class Controller():
    ...
    def update(self, x):
        ...
        self.err_int = np.clip(self.err_int, -0.3, 0.3)
...
class Controller_Node(Node):
    def __init__(self, detection_callback_group, timer_callback_group, global_pos_callback_group):
        ...
        self.global_pos_sub = self.create_subscription(VehicleLocalPosition, '/fmu/out/vehicle_local_position', self.global_pos_callback, qos_prof, callback_group=detection_callback_group)
        self.global_pos_sub
        ...
        # self.detector = pyyolo.YOLO('/home/patryk/darknet/cfg/yolov7-tiny.cfg', '/home/patryk/yolov7-tiny.weights', '/home/patryk/darknet/cfg/coco.data')
        ...
        self.roll_controller = Controller(IMG_W_C, 0.30, float('inf'), 1.30, MIN_ROLL, MAX_ROLL) #x, d 0.005
        self.pitch_controller = Controller(IMG_H_C, 0.30, float('inf'), 1.30, MIN_PITCH, MAX_PITCH) #S
        self.throttle_controller = Controller(TARG_SR*IMG_S, 0.45, float('inf'), 4.60, MIN_THROTTLE, MAX_THROTTLE) #0.4, inf, 0 - last working 
	# settings 0.3, 0, 0.2,  i=0.3
    ...
    def img_callback(self, data):
        ...
        #publish image post detection
        #self.post_det_img_publisher.publish(self.bridge.cv2_to_imgmsg(self.current_frame, 'rgb8'))
        #self.get_logger().info(f'{tt-time.time()}')
        ...
        if target_detected and not self.is_manual:
            self.target_control_vals[0] = -self.roll_controller.update(np.sum(self.det_x_queue)/X_QUEUE_LEN)
            self.target_control_vals[2] = self.throttle_controller.update(2*TARG_SR*IMG_S - np.sum(self.det_s_queue)/S_QUEUE_LEN) #2*TARG_SR*IMG_S - self.det_s
            #if np.abs(self.det_x) < 200 and np.abs(self.det_y) < 100:
            self.target_control_vals[1] = self.pitch_controller.update(np.sum(self.det_y_queue)/Y_QUEUE_LEN)
        
        else:
            self.target_control_vals = [0, 0, (MIN_THROTTLE+MAX_THROTTLE)/2, 0]

        #else:
            #self.target_control_vals[1] = 0
    ...
    def _control_update(self):
        ...
        self.get_logger().info(f"{self.global_pos_sub, self.subscription}")
    ...
    def update_log(self):
        self.log.write(f"{time.time() - self.log_start_time} {self.det_x} {self.det_y} {self.det_s} {self.curr_control_vals[0]} {self.curr_control_vals[1]} {self.curr_control_vals[2]} {self.curr_control_vals[3]} {int(self.is_manual)} {self.global_x} {self.global_y} {self.global_z}\n")  
