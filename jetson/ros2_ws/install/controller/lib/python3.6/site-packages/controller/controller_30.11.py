...
class Controller_Node(Node):
    def __init__(self, detection_callback_group, timer_callback_group):
        ...
        self.roll_controller = Controller(IMG_W_C, 0.32, float('inf'), 0.4, MIN_ROLL, MAX_ROLL) #x, d 0.005
        self.pitch_controller = Controller(IMG_H_C, 0.35, float('inf'), 0.35, MIN_PITCH, MAX_PITCH) #S
        self.throttle_controller = Controller(TARG_SR*IMG_S, 0.45, float('inf'), 0.5, MIN_THROTTLE, MAX_THROTTLE) #0.4, inf, 0 - last working 
    ...
    def img_callback(self, data):
        ...
        if target_detected:
            ...
            self.target_control_vals[2] = self.throttle_controller.update(2*TARG_SR*IMG_S - self.det_s) #2*TARG_SR*IMG_S - self.det_s
            #if np.abs(self.det_x) < 200 and np.abs(self.det_y) < 100:
            self.target_control_vals[1] = self.pitch_controller.update(self.det_y)
